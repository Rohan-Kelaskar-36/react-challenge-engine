...
type CourseProgress = {
  ...
  challenges: Record<
    string,
    { passed: boolean; score: number; lastRun: string | null }
  >;
};
...
export default function App() {
  ...
  const [view, setView] = useState<"courses" | "challenges" | "detail">(
    "courses",
  );
  ...
  const fetchProgress = useCallback(async () => {
    try {
      ...
      if (
        ...
        setError(
          "Dashboard API not reachable. Start it from repo root: npm run dashboard:api (or npm run dashboard:dev for UI + API).",
        );
      } else {
        setError(msg);
      }
    ...
    async (page: number, signal?: AbortSignal) => {
      ...
      try {
        const r = await fetchWithTimeout(
          `${API}/courses?page=${page}&limit=20`,
  ...
  const fetchChallenges = useCallback(
    async (courseId: string, page: number, signal?: AbortSignal) => {
      setLoadingChallenges(true);
      try {
        const r = await fetchWithTimeout(
          `${API}/courses/${courseId}/challenges?page=${page}&limit=50`,
          { signal, timeoutMs: FETCH_TIMEOUT_MS },
        );
        const data = await r.json().catch(() => ({}));
        if (!r.ok) {
  ...
  const fetchDetail = useCallback(
    async (courseId: string, challengeId: string, signal?: AbortSignal) => {
      setLoadingDetail(true);
      try {
        const r = await fetchWithTimeout(
          `${API}/courses/${courseId}/challenges/${challengeId}`,
          { signal, timeoutMs: FETCH_TIMEOUT_MS },
        );
        const data = await r.json().catch(() => ({}));
        if (!r.ok) {
  ...
  useEffect(() => {
    ...
    if (!courseId) return;
    const controller = new AbortController();
    fetchWithTimeout(`${API}/courses/${courseId}`, {
      signal: controller.signal,
      timeoutMs: 10000,
    })
      .then((r) =>
  ...
  const runReview = async (courseId: string, challengeId: string) => {
    ...
    try {
      ...
      try {
        ...
        setError(
          r.ok
            ? "Review server returned invalid response."
            : `Review failed (${r.status}). Is the dashboard API running? (npm run dashboard:api or npm run dashboard:dev)`,
        );
        return;
      }
      if (!r.ok || !data.ok) {
        setError(
          data.error ||
            (r.status ? `Review failed (${r.status})` : "Review failed"),
        );
        return;
      }
            ...
            if (prog.lastUpdated !== initialLastUpdated) {
              ...
              if (selectedCourse?.id === courseId) {
                const res = await fetch(
                  `${API}/courses/${courseId}/challenges?page=${challengesPage}&limit=50`,
                );
      ...
      if (
        ...
        setError(
          "Cannot reach review server. Start the dashboard API: npm run dashboard:api (or npm run dashboard:dev for UI + API).",
        );
      } else {
        ...
        <>
          <section className="progress-summary">
            ...
            <div className="progress-card">
              ...
              <span>
                {pathway.completedChallenges ?? 0} /{" "}
                {pathway.totalChallenges ?? 0}
              </span>
            </div>
            <div className="progress-card">
          ...
          <section className="pathway-progress">
            ...
            <div className="progress-bar-container">
              ...
              <div className="progress-bar-label">
                {pathway.completedChallenges ?? 0} /{" "}
                {pathway.totalChallenges ?? 0} challenges completed (
                {Math.round(pathway.completionPercentage ?? 0)}%)
              </div>
            </div>
          </section>
          {/* Celebrations are visual only; no sound. */}
          {pathway.totalChallenges != null &&
            pathway.totalChallenges > 0 &&
            pathway.completedChallenges === pathway.totalChallenges && (
              <div
                className="celebration celebration-pathway"
                role="status"
                aria-live="polite"
              >
                <i
                  className="fa-solid fa-trophy celebration-icon"
                  aria-hidden
                />
                <div>
                      ...
                      <div key={c.id} className="card">
                        <div style={{ flex: 1 }}>
                          ...
                          <div className="meta">
                            Score:{" "}
                            {c.averageScore != null
                              ? Math.round(c.averageScore)
                              : "—"}
                            % · Completion:{" "}
                            {c.completionPercentage != null
                              ? Math.round(c.completionPercentage)
                              : "—"}
                            %
                          </div>
                          <div
                            className="course-progress-bar-container"
                        ...
                        <button type="button" onClick={() => openChallenges(c)}>
                          View challenges
                        </button>
                      </div>
                    );
            ...
            <section className="course-progress-bar">
              <div className="course-progress-stats">
                ...
                <div className="stat-item">
                  ...
                  <span className="stat-value">
                    {progress.courses[selectedCourse.id].completedChallenges ||
                      0}{" "}
                    / {progress.courses[selectedCourse.id].totalChallenges || 0}
                  </span>
                </div>
                <div className="stat-item">
                  ...
                  <span className="stat-value">
                    {Math.round(
                      progress.courses[selectedCourse.id].averageScore || 0,
                    )}
                    %
                  </span>
                </div>
              ...
              <div className="course-progress-bar-visual">
                <div className="progress-bar-slim">
                  <div
                    ...
                    style={{
                      width: `${progress.courses[selectedCourse.id].completionPercentage || 0}%`,
                    }}
                  />
                </div>
              </div>
          ...
          {progress.courses[selectedCourse.id] &&
            (() => {
              ...
              return total > 0 && completed === total ? (
                <div
                  className="celebration celebration-course"
                  role="status"
                  aria-live="polite"
                >
                  <i
                    className="fa-solid fa-star celebration-icon"
                    aria-hidden
                  />
                  <div>
                    <strong>Course complete!</strong>
                    <p>
                      All challenges in {selectedCourse.name} passed. Ready for
                      the next course?
                    </p>
                  </div>
                </div>
            ...
            <>
              <div className="card-list">
                {challenges
                  ...
                  .map((ch) => {
                    const score =
                      ch.score != null ? Math.round(ch.score) : null;
                    ...
                    return (
                      <div key={ch.id} className="card">
                        <div style={{ flex: 1 }}>
                          ...
                          <div className="meta">
                            <span
                              className={`badge ${ch.passed ? "passed" : ch.score != null ? "failed" : "pending"}`}
                            >
                              {ch.passed ? (
                                <>
                                  <i
                            ...
                            {ch.lastRun &&
                              ` · Last run: ${new Date(ch.lastRun).toLocaleString()}`}
                          </div>
                          {score != null && (
                            <div
                              className="challenge-score-progress"
                              style={{ marginTop: "0.5rem" }}
                            >
                              <div
                                className={`score-progress-bar ${scoreColorClass}`}
                              >
                                <div
              ...
              {challengesTotalPages > 1 && (
                <div className="pagination">
                  <button
                    ...
                    onClick={() => setChallengesPage((p) => p - 1)}
                  >
                    Previous
                  </button>
                  <span>
      ...
      {view === "detail" &&
        ...
        !loadingDetail && (
          <div className="detail-panel">
            ...
            <div className="detail-header">
              <div>
                ...
                <div className="detail-meta">
                  <span
                    className={`badge ${detail.passed ? "passed" : detail.score != null ? "failed" : "pending"}`}
                  >
                    {detail.passed ? (
                      <>
                        <i className="fa-solid fa-check" aria-hidden /> Passed
                      </>
                    ) : detail.score != null ? (
                      <>
                        <i className="fa-solid fa-xmark" aria-hidden /> Not
                        passed
                      </>
                    ) : (
                      <>
                        <i className="fa-regular fa-circle" aria-hidden /> Not
                        run
                      </>
                    )}
                  </span>
                  ...
                  {detail.lastRun && (
                    <span className="detail-score">
                      Last run: {new Date(detail.lastRun).toLocaleString()}
                    </span>
                  )}
                </div>
                {(() => {
                  const skills =
                    detail.skills ??
                    (detail as { metadata?: { skills?: string[] } }).metadata
                      ?.skills ??
                    [];
                ...
                {detail.score != null && (
                  ...
                  >
                    <div
                      className={`score-progress-bar ${getScoreColorClass(detail.score)}`}
                    >
                      <div
                        className="score-progress-fill"
              ...
              >
                {runningReview === `${selectedCourse.id}/${detail.id}` ? (
                  ...
                  <>
                    <i className="fa-solid fa-play btn-icon" aria-hidden /> Run
                    review
                  </>
                )}
              </button>
            ...
            {detail.passed && (
              <div
                className="celebration celebration-challenge"
                role="status"
                aria-live="polite"
              >
            ...
            {detail.instructions && (
              <section className="instructions-section">
                ...
                >
                  ...
                  <span
                    className={`collapse-icon ${instructionsCollapsed ? "collapsed" : ""}`}
                  >
                    ▼
                  </span>
                </h3>
                ...
                <div className="review-results-content">
                  <div className="results-summary">
                    <div className="results-summary-row">
                      ...
                      <span>
                        {typeof detail.result.totalScore === "number"
                          ? `${Math.round(detail.result.totalScore)}%`
                          : "—"}
                      </span>
                    </div>
                    <div className="results-summary-row">
                      ...
                      <span
                        className={`badge ${detail.result.passed ? "passed" : "failed"}`}
                      >
                  ...
                  {(() => {
                    ...
                    if (detail.result.testResults) {
                      const score =
                        typeof detail.result.testResults.score === "number"
                          ? Math.round(detail.result.testResults.score)
                          : 0;
                      layers.push({
                        key: "testResults",
                        score,
                        component: (
                          <section
                            key="testResults"
                            className={`result-layer collapsible ${score === 100 ? "result-layer-perfect" : ""}`}
                          >
                            <h4
                              className="result-layer-header"
                              onClick={(e) => {
                                const section =
                                  e.currentTarget.closest(".result-layer");
                                if (section)
                            ...
                            <div className="result-layer-content">
                              <div className="result-layer-score">
                                Score: {score}%
                                {detail.result.testResults.totalTests !=
                                  null && (
                                  <>
                                    {" "}
                                    ·{" "}
                                    {detail.result.testResults.passedTests ?? 0}
                                    /{detail.result.testResults.totalTests}{" "}
                                    passed
                                  </>
                                )}
                              ...
                              {detail.result.testResults.details?.map(
                                (suite, i) => (
                                  <div key={i}>
                    ...
                    if (detail.result.lintResults) {
                      const score =
                        typeof detail.result.lintResults.score === "number"
                          ? Math.round(detail.result.lintResults.score)
                          : 0;
                      layers.push({
                        key: "lintResults",
                        score,
                        component: (
                          ...
                          >
                            ...
                            <div className="result-layer-content">
                              <div className="result-layer-score">
                                Score: {score}%
                                {detail.result.lintResults.errors != null &&
                                  detail.result.lintResults.warnings !=
                                    null && (
                                    <>
                                      {" "}
                                      · {detail.result.lintResults.errors}{" "}
                                      error(s),{" "}
                                      {detail.result.lintResults.warnings}{" "}
                                      warning(s)
                                    </>
                              ...
                              {detail.result.lintResults.details?.map(
                                (file, i) =>
                                  file.messages?.length ? (
                                    <div key={i} className="result-issue">
                                      ...
                                      {file.messages.map((m, j) => (
                                        <p key={j} className="result-issue-msg">
                    ...
                    if (detail.result.architectureResults) {
                      const score =
                        typeof detail.result.architectureResults.score ===
                        "number"
                          ? Math.round(detail.result.architectureResults.score)
                          : 0;
                      layers.push({
                        ...
                        component: (
                          ...
                          >
                            ...
                            <div className="result-layer-content">
                              ...
                              {detail.result.architectureResults.patternsMissing
                                ?.length ? (
                                <>
                                  <p className="result-issue-msg">
                                    Patterns expected but not found:{" "}
                                    <strong>
                                      {[
                                        ...new Set(
                                    ...
                                    . Add these patterns to your code as
                                  ...
                                  >
                                    Hint: <code>useState</code> = React state
                                    hook; <code>props</code> = component props;{" "}
                                    <code>functionalComponent</code> = function
                                    component (not class).
                                  </p>
                                </>
                              ) : null}
                              {detail.result.architectureResults.details?.map(
                                (d, i) =>
                                  ...
                                  d.patternsFound?.length ? (
                                    <div key={i} className="result-issue">
                                      ...
                                      {d.patternsMissing?.length ? (
                                        <p className="result-issue-msg">
                                          Missing:{" "}
                                          {d.patternsMissing.join(", ")}
                                        </p>
                                      ) : null}
                                      {d.patternsFound?.length ? (
                                        <p className="result-issue-msg">
                                          Found: {d.patternsFound.join(", ")}
                                        </p>
                                      ) : null}
                                    </div>
                                  ) : null,
                    ...
                    if (detail.result.bestPracticesResults) {
                      const score =
                        typeof detail.result.bestPracticesResults.score ===
                        "number"
                          ? Math.round(detail.result.bestPracticesResults.score)
                          : 0;
                      layers.push({
                            ...
                            <div className="result-layer-content">
                              ...
                              {(() => {
                                ...
                                return uniqueIssues.length > 0 ? (
                                    ...
                                    {uniqueIssues.map((issue, i) => {
                                      const msg =
                                        typeof issue === "string"
                                          ...
                                          : ((issue as { message?: string })
                                              .message ?? "Issue");
                                      const file = (issue as { file?: string })
                                        .file;
                                      return file ? (
                                        <div key={i} className="result-issue">
                                          <strong>{file}</strong>
                                        ...
                                        <p key={i} className="result-issue-msg">
                                          • {msg}
                                        </p>
                                      );
                                    })}
                                  ...
                                  >
                                    ✅ All best practices requirements met
                                  </p>
                                ) : null;
                              })()}
                            </div>
                    ...
                    if (detail.result.e2eResults) {
                      const score =
                        typeof detail.result.e2eResults.score === "number"
                          ? Math.round(detail.result.e2eResults.score)
                          : 0;
                      layers.push({
                        key: "e2eResults",
                        score,
                        component: (
                          ...
                          >
                            ...
                            <div className="result-layer-content">
                              <div className="result-layer-score">
                                Score: {score}%
                                {detail.result.e2eResults.passedTests != null &&
                                  detail.result.e2eResults.totalTests !=
                                    null && (
                                    <>
                                      {" "}
                                      · {detail.result.e2eResults.passedTests}/
                                      {detail.result.e2eResults.totalTests}{" "}
                                      passed
                                    </>
                                  )}
                              ...
                              {detail.result.e2eResults.error ? (
                                <p className="result-layer-error">
                                  {
                                    detail.result.e2eResults.error.split(
                                      "\n",
                                    )[0]
                                  }
                                </p>
                              ...
                              {(
                                detail.result.e2eResults as {
                                  details?: unknown[];
                                }
                              ).details?.length ? (
                                <p className="result-issue-msg">
                                  See test output for failed steps.
                                </p>
                              ) : null}
                              {score === 100 &&
                                !detail.result.e2eResults.error && (
                    ...
                    if (detail.result.aiReviewResults || detail.aiFeedback) {
                      const ai =
                        detail.result.aiReviewResults ||
                        (detail.aiFeedback as Record<string, unknown>);
                      if (ai && !ai.error) {
                        const score =
                        ...
                        layers.push({
                          ...
                          component: (
                            <section
                              key="aiReviewResults"
                              className={`result-layer result-layer-ai collapsible ${score === 100 ? "result-layer-perfect" : ""}`}
                            >
                              <h4
                                className="result-layer-header"
                                onClick={(e) => {
                              ...
                              <div className="result-layer-content">
                                {ai.error ? (
                                  ...
                                  <>
                                    ...
                                    {ai.overall && (
                                      <p className="result-ai-overall">
                                        {ai.overall as string}
                                      </p>
                                    )}
                        ...
                        layers.push({
                          ...
                          component: (
                            <section
                              key="aiReviewResults"
                              className="result-layer result-layer-ai collapsible"
                            >
                              <h4
                                className="result-layer-header"
                                onClick={(e) => {
